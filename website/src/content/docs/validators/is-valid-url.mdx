...
| `protocols` | `string \| readonly string[]`   | No       | Allowlist of accepted protocols (e.g., `"https"` or `["http", "https"]`). Matched case-insensitively, no trailing `:` |
...
- Protocol names are normalized to lowercase, so `["HTTPS"]` matches `https://...`.
