...
```ts
function isValidPhone({ phone }: IsValidPhoneParams): boolean
```
...
- Validation is structural — it does not check whether the country code, area code, or subscriber number actually exist. For carrier-grade validation, use a library backed by phone metadata.
